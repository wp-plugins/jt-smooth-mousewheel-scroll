jQuery(document).ready(function($){
    ...
    var optionDevice = jtCgs_ss_parmas.device,
        optionSize = parseInt(jtCgs_ss_parmas.size),
        optionSpeed = parseInt(jtCgs_ss_parmas.speed),
        optionEasingCheck = jtCgs_ss_parmas.easingCheck,
        optionEasing = jtCgs_ss_parmas.easing;
        
        if(optionEasingCheck == "easing_ok" && optionEasing == "") optionEasing = "easeOutQuart";
        if(optionEasingCheck == "") optionEasing = "swing";
        
    
    if(optionDevice == "all_device") jt_smoothwheel_scroll();
    
    if(optionDevice == "no_mac"){
        var pcFilter = "mac|macintel"; 
        if (pcFilter.indexOf(navigator.platform.toLowerCase()) < 0) jt_smoothwheel_scroll();
    }
    
    function jt_smoothwheel_scroll(){"use strict";
